...
from commons.engine.logger import create_logger
from commons.losses.ae_loss import AELoss
from tokenizer.vfmae.vfmae import AE_models
from commons.engine.lr_scheduler import build_scheduler
...
from commons.engine.misc import is_main_process, all_reduce_mean, get_rank, get_world_size
...
def main(args):
    ...
    logger.info(f"VQ Model Parameters: {sum(p.numel() for p in ae_model.parameters()):,}")
    if args.ema:
        ema = deepcopy(ae_model).to(device)  # Create an EMA of the model for use after training
    ...
    #* Setup optimizer
    optimizer = torch.optim.Adam(ae_model.parameters(), lr=args.lr, betas=(args.beta1, args.beta2))
    ...
    if args.ae_ckpt:
        ...
        m1, u1 = ae_model.load_state_dict(checkpoint["model"],strict=False)
        ...
        if not args.finetune:
            train_steps = checkpoint["steps"] if "steps" in checkpoint else int(args.ae_ckpt.split('/')[-1].split('.')[0])
            start_epoch = round(train_steps / int(len(dataset) / args.global_batch_size))
        else:
            train_steps = 0
            start_epoch = 0           
        del checkpoint
        logger.info(f"Resume training from checkpoint: {args.ae_ckpt}")
        logger.info(f"Initial state: steps={train_steps}, epochs={start_epoch}")
    else:
        train_steps = 0
        start_epoch = 0
        if args.ema:
            update_ema(ema, ae_model, decay=0)  # Ensure EMA is initialized with synced weights

    if args.compile:
        logger.info("compiling the model... (may take several minutes)")
        ae_model = torch.compile(ae_model) # requires PyTorch 2.0        
    
    ae_model = DDP(ae_model.to(device), device_ids=[args.gpu], find_unused_parameters=False)
    ae_model.train()
    if args.ema:
    ...
    if isinstance(ae_model, DDP):
        ae_model.module.freeze_visual_encoder()
    else:
        ae_model.freeze_visual_encoder()
        ...
        with tqdm(loader, dynamic_ncols=True, disable=not is_main_process()) as train_dl:
            for i, (imgs, labels) in enumerate(train_dl):
                ...
                with torch.cuda.amp.autocast(dtype=ptdtype):  
                    recons_imgs, posterior = ae_model(imgs)

                    loss_gen, sim_loss = vq_loss(posterior, imgs, recons_imgs, optimizer_idx=0, global_step=train_steps+1, 
                                    last_layer=ae_model.module.last_layer(), 
                                    log_every=args.log_every, args=args,)

                scaler.scale(loss_gen).backward()
                if args.max_grad_norm != 0.0:
                    scaler.unscale_(optimizer)
                    torch.nn.utils.clip_grad_norm_(ae_model.parameters(), args.max_grad_norm)
                
                g_sched.step_update(train_steps + 1)
                scaler.step(optimizer)
                scaler.update()
                if args.ema:
                    update_ema(ema, ae_model.module._orig_mod if args.compile else ae_model.module)

                #* Discriminator training 
                start_discr = train_steps >= args.disc_start
                if start_discr:  
                    optimizer_disc.zero_grad()
                    with torch.cuda.amp.autocast(dtype=ptdtype):
                        loss_disc = vq_loss(posterior, imgs, recons_imgs, optimizer_idx=1, global_step=train_steps+1,
                                            log_every=args.log_every, args = args)
                ...
                avg_loss = all_reduce_mean(loss_gen + loss_disc) if start_discr else all_reduce_mean(loss_gen)
                loss_gen = all_reduce_mean(loss_gen)
                loss_discr = all_reduce_mean(loss_disc) if start_discr else 0 * avg_loss
                ...
                if (train_steps > 0) & (train_steps % args.ckpt_every == 0) & is_main_process():
                    if isinstance(ae_model, DDP):
                        model_weight = ae_model.module
                    else:
                        model_weight = ae_model
    ...
    ae_model.eval()  # important! This disables randomized embedding dropout
...
if __name__ == "__main__":
    ...
    parser.add_argument("--embed-dim", type=int, default=16, help="codebook dimension for vector quantization")
