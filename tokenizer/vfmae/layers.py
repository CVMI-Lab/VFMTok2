...
from commons.modules.mlp import build_mlp
from commons.engine.ema import requires_grad
from commons.engine.util import instantiate_from_config
from commons.modules.transformer import VanillaDeformableTransformerEncoder
...
class Encoder(nn.Module):
    ...
    def __init__(self, image_size, layer_type, n_carrier,
        ...
        self.transformer = VanillaDeformableTransformerEncoder(dim, depth, num_feature_levels=4,enc_n_points=enc_n_points)
    ...
    def forward(self, imgs):
        ...
        for i, layer in enumerate(self.projectors):
            hidden_state = latent[i][:, 5:] + latent[i][:,:1]
            kv_feature = self.norm_layers[i](layer(hidden_state))
        ...
        hidden_state = latent[-1]
        latent = torch.cat((hidden_state[:, :1], hidden_state[:, 5:]), dim=1)
...
class Decoder(nn.Module):
    ...
    def single_forward(self, conds, z, norm, reshape=False):
        ...
        x = self.transformer(x,) if reshape else self.dino_transformer(x)
        
        if reshape:
            z = rearrange(x[:, n_c + self.num_tokens:n_c + num_patches], 'b (h w) c -> b c h w', h=H, w=W)
        else:
            z = x[:, n_c:]
        return z
    ...
    def forward(self, slots):
        ...
        cc = torch.cat((cls_token + z_pos[:, :1], z + z_pos[:, 1:]), dim=1)

        z = self.single_forward(memory, cc, self.norm_post1, True)

        if self.training:
            pos_embeds = self.interpolate_pos_encoding(z_pos, W, H)
            dinov2 = self.dino_mask + pos_embeds[:, 1:]
            dinov2 = repeat(dinov2, 'f ... -> (b f) ...', b=bs)
            dinov2 = torch.cat((cls_token + self.position_embedding[:,:1], dinov2), dim=1)
