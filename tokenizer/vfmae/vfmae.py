...
class AutoencoderAE(nn.Module):
    ...
    def __init__(self, config: ModelArgs):
        ...
        self.slots_quant = Projector(config)

        self.post_slots_quant = nn.Linear(embed_dim, config.z_channels)
    ...
    def encode(self, x):
        ...
        posterior = self.slots_quant(slots)

        return (posterior, latent)

    def encode_imgs(self, inp):
        
        bs = inp.size(0)
        posterior, _ = self.encode(inp)
        posterior = posterior.flatten(2).transpose(1, 2).contiguous()

        H = W = int(math.sqrt(posterior.size(1)))
        posterior = rearrange(posterior, 'b (h w) c -> b c h w', b=bs, h=H, w=W).contiguous()
        return posterior

    def decode(self, posterior):
        
        slots = rearrange(posterior, 'b c k n -> b (k n) c')
    ...
    def decode_to_imgs(self, codes, tgt_size=256):

        results, _ = self.decode(codes)
    ...
    def forward(self, input):
        ...
        (posterior, latent) = self.encode(input)

        dec, dinov2 = self.decode(posterior)

        return (dec, latent, dinov2), posterior
...
class Projector(nn.Module):
    ...
    def __init__(self, config: ModelArgs):
        ...
        super().__init__()
        # end
        self.norm_out = Normalize(config.z_channels)
        self.conv_out = nn.Conv2d(config.z_channels, 1 * config.embed_dim, 1)
    
    def forward(self, h):
        
        h = rearrange(h, 'b (k n) c-> b c k n', k=1,)
        h = self.norm_out(h)
...
def AE_16(**kwargs):
    ...
    return AutoencoderAE(ModelArgs(encoder_ch_mult=[1, 1, 2, 2, 4], decoder_ch_mult=[1, 1, 2, 2, 4], **kwargs))

AE_models = {'AE-16': AE_16}