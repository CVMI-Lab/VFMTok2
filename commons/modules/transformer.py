...
from .mlp import SwiGLUFFNFused, build_mlp
from .attention import Attention3, CrossAttention
from .deformable_transformer import DeformableTransformerEncoderLayer
from .attention import MemoryEfficientCrossAttention, XFORMERS_IS_AVAILBLE
from .deformable_transformer import VanillaDeformableTransformerEncoderLayer
...
class VanillaDeformableTransformerEncoder(nn.Module):

    def __init__(self, dim, depth, dim_feedforward=1024, dropout=0.,
                 activation="relu", num_feature_levels = 2, n_heads=8, enc_n_points=9):
        super().__init__()
        self.num_feature_levels = num_feature_levels
        encoder_layer = VanillaDeformableTransformerEncoderLayer(dim, min(4 * dim, dim_feedforward),       \
                            dropout, activation, num_feature_levels, n_heads, enc_n_points)
        self.layers = _get_clones(encoder_layer, depth)
        self.num_layers = depth
