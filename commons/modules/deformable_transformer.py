...
from .attention import Attention3, Attention4
...
class VanillaDeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
                 d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4):
        ...
        # self attention
        self.self_attn = Attention4(d_model, n_heads, dropout=dropout)
        self.dropout4 = nn.Dropout(dropout)
        self.norm3 = nn.LayerNorm(d_model)
    ...
    def forward(self, src, pos, reference_points, kv_feature, input_spatial_shape, level_start_index):
        ...
        with torch.autocast('cuda', dtype=torch.float32, enabled=True):
            tgt2 = self.self_attn(self.with_pos_embed(src, pos),)
        src = src + self.dropout4(tgt2)
        src = self.norm3(src)
