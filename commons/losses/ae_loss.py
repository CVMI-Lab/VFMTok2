...
from commons.engine.misc import get_world_size
...
class AELoss(nn.Module):
    def __init__(self, disc_start, disc_loss="hinge", disc_dim=64, disc_type='patchgan', image_size=256,
                 ...
                 codebook_weight=1.0, perceptual_weight=1.0, kl_weight = 1e-6,
    ):
        super().__init__()
        # discriminator loss
    ...
    def forward(self, posterior, inputs, outputs, optimizer_idx, global_step, last_layer=None, 
                logger=None, log_every=100, args = None,):
        ...
        if optimizer_idx == 0:
            ...
            start_discr = global_step >= args.disc_start
            generator_adv_loss, disc_adaptive_weight, disc_weight  = 0 * rec_loss, 0, 0
            if start_discr:
            ...
            # kl_loss = torch.sum(kl_loss) / kl_loss.size(0)
            
            loss = self.rec_weight * rec_loss + \
                self.perceptual_weight * p_loss + \
                disc_adaptive_weight * disc_weight * generator_adv_loss +  sim_loss.mean()
                # self.kl_weight * kl_loss + sim_loss.mean()
